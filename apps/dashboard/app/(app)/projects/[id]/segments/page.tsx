...
function ManageSegmentDialog({
  ...
  const { data: subscribersData } = useSubscribers(projectId, 1, 100);
  const { mutate: addSubscriber, isPending: isAdding } =
    useAddSubscriberToSegment(projectId);
  const { mutate: removeSubscriber, isPending: isRemoving } =
    useRemoveSubscriberFromSegment(projectId);
...
export default function ProjectSegmentsPage() {
  ...
  const { data: segments, isLoading } = useSegments(projectId);
  const { mutate: createSegment, isPending: isCreating } =
    useCreateSegment(projectId);
  const { mutate: deleteSegment } = useDeleteSegment(projectId);
    ...
    <div className="space-y-6">
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            A list of all subscriber segments for this project.
          </CardDescription>
        </CardHeader>
        <CardContent>
